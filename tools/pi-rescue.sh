...
runcmd:
  ...
  - [ bash, -c, "mkdir -p /etc/soluna && printf '%s\\\\n' '$AP_PSK' > /etc/soluna/ap.psk && chmod 600 /etc/soluna/ap.psk" ]
  - [ bash, -c, "grep -q SOLUNA_AP_PSK /etc/soluna/agent.env 2>/dev/null && sed -i 's/^SOLUNA_AP_PSK=.*/SOLUNA_AP_PSK=$AP_PSK/' /etc/soluna/agent.env || echo SOLUNA_AP_PSK=$AP_PSK >> /etc/soluna/agent.env" ]
  - [ bash, -c, "P='$UPSTREAM'; if [ -z \\"\\$P\\" ]; then P=\\$(nmcli -t -f NAME,TYPE con show | awk -F: '\\$2 ~ /wireless|wifi/ && \\$1 != \\"soluna-ap\\" {print \\$1; exit}'); fi; [ -n \\"\\$P\\" ] && nmcli con up \\"\\$P\\" || true" ]
  - [ bash, -c, "for i in \\$(seq 1 30); do curl -fsS -m 5 https://github.com >/dev/null 2>&1 && break; sleep 5; done; curl -fsSL https://raw.githubusercontent.com/yukihamada/soluna-surround/master/tools/pi-setup.sh | SUDO_USER=pi bash > /var/log/soluna-rescue.log 2>&1 || true" ]
